...
public class YouthSkillInfoColumn extends JLabel implements IHOTableEntry {
    ...
    private static final int bar_width = 8 * UserParameter.instance().fontSize; // pixels to display skill range from 0
                                                                                // to 9 (8.3)
    ...
    private Color getSkillRating(YouthSkillInfo skillInfo) {
        if (skillInfo.getMax() != null)
            return cellSkillRatingColor[Math.max(0, skillInfo.getMax() - 4)];
        if (skillInfo.getMaximumPotential() < 6)
            return cellSkillRatingColor[Math.max(0, skillInfo.getMaximumPotential() - 4)];
        return Color.white;
    }
    ...
    private String createToolTipText() {
        ...
        return "<html>" +
                ...
                String.format(TranslationFacility.tr("ls.youth.skill.start") + ": %.2f<br>",
                        this.skillInfo.getStartValue())
                +
                String.format(TranslationFacility.tr("ls.youth.skill.current") + ": %.2f<br>",
                        this.skillInfo.getCurrentValue())
                +
                (this.skillInfo.getPotential17Value() != null
                        ? String.format(TranslationFacility.tr("ls.youth.skill.17") + ": %.2f<br>",
                                this.skillInfo.getPotential17Value())
                        : "")
                +
                TranslationFacility.tr("ls.youth.skill.max") + ": "
                + (this.skillInfo.getMax() != null ? this.skillInfo.getMax() : unknown) + "<br>" +
                (this.skillInfo.isMaxReached() ? TranslationFacility.tr("ls.youth.skill.ismaxreached") + "<br>" : "") +
                TranslationFacility.tr("ls.youth.skill.startlevel") + ": "
                + (this.skillInfo.getStartLevel() != null ? this.skillInfo.getStartLevel() : unknown) + "<br>" +
                TranslationFacility.tr("ls.youth.skill.currentlevel") + ": "
                + (this.skillInfo.getCurrentLevel() != null ? this.skillInfo.getCurrentLevel() : unknown) +
                (this.skillInfo.isTop3() != null && this.skillInfo.isTop3()
                        ? "<br>" + TranslationFacility.tr("ls.youth.skill.istop3")
                        : "")
                +
                "</html>";
    }
    ...
    private Color getColorTrainedBar() {
        if (this.skillInfo.isMaxReached())
            return Color_MaxReached;
        if (this.skillInfo.isMaxLevelReached())
            return Color_MaxLevelReached;
