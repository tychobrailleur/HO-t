...
public class TransfersPanel extends JPanel implements IRefreshable {
	...
	public TransfersPanel() {
		...
		this.players = HOModelManager.instance().getModel().getCurrentPlayers();
		this.oldplayers = HOModelManager.instance().getModel().getFormerPlayers();
	...
	public void refresh() {
        ...
        final List<Player> tmp = new Vector<>(HOModelManager.instance().getModel().getCurrentPlayers());
		tmp.removeAll(this.players);
		final List<Player> allOutdated = new Vector<>(tmp);

		// Check for outdated old players.
		tmp.clear();
		tmp.addAll(HOModelManager.instance().getModel().getFormerPlayers());
		tmp.removeAll(this.oldplayers);
		allOutdated.addAll(tmp);

        allOutdated.removeIf(player -> player.getPlayerId() < 0);
		boolean success = false;
		if ((!allOutdated.isEmpty()) && !HOModelManager.instance().getModel().getBasics().isNationalTeam() &&  (DBManager.instance().getTransfers(0, true, true).isEmpty())) {
			success = XMLParser.updateTeamTransfers(HOModelManager.instance().getModel().getBasics().getTeamId());
		}
