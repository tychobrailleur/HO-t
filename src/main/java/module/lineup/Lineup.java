...
public class Lineup {
    ...
    /**
     * The rating revision number gets incremented each time a rating relevant
     * property gets changes. This is used by rating calculating caches, that
     * will reset, when they see a new revision number.
     */
    ...
    public String getAttitudeName(int attitude) {
        return switch (attitude) {
            case IMatchDetails.EINSTELLUNG_NORMAL ->
                TranslationFacility.tr("ls.team.teamattitude_short.normal");
            case IMatchDetails.EINSTELLUNG_PIC ->
                TranslationFacility.tr("ls.team.teamattitude_short.playitcool");
            case IMatchDetails.EINSTELLUNG_MOTS ->
                TranslationFacility.tr("ls.team.teamattitude_short.matchoftheseason");
            default ->
                TranslationFacility.tr("Unbestimmt");
        };
    }
...
//		}
     ...
     *
     * @param location New value of property m_sHeimspiel.
     */
    ...
    public final short getLocation() {
        if (m_sLocation == -1 && !isUpcomingMatchLoaded()) {
            getUpcomingMatch();
        }
        return m_sLocation;
    }
    ...
    public final Weather getWeather() {
        if (m_cWeather == null || (m_cWeather == Weather.NULL && !isUpcomingMatchLoaded())) {
            getUpcomingMatch();
        }
        return m_cWeather;
    }
    ...
    public final Weather.Forecast getWeatherForecast() {
        if (this.m_cWeatherForecast == Weather.Forecast.NULL && !isUpcomingMatchLoaded()) {
            getUpcomingMatch();
        }
        return m_cWeatherForecast;
    }

    private boolean isUpcomingMatchLoaded() {
        return m_iArenaId >= 0;
    }
    ...
    public final MatchLineupPosition getPositionByPlayerId(int playerid, boolean includeRemovedPlayers) {
        ...
        if (ret == null) {
            ret = getPositionByPlayerId(playerid, m_vBenchPositions);
        }
        if (includeRemovedPlayers) {
            if (ret == null) {
                ret = getPositionByPlayerId(playerid, replacedPositions);
            }
            if (ret == null) {
    ...
    public final Vector<MatchLineupPosition> getAllPositions() {
        ...
        if (m_vFieldPositions != null) {
            ret.addAll(m_vFieldPositions);
        }
        if (m_vBenchPositions != null) {
            ret.addAll(m_vBenchPositions);
        }
        if (replacedPositions != null) {
            ret.addAll(replacedPositions);
        }
        if (redCardedPositions != null) {
            ret.addAll(redCardedPositions);
        }
        if (penaltyTakers != null) {
            ret.addAll(penaltyTakers);
        }
        if (captain != null) {
            ret.add(captain);
        }
        if (setPiecesTaker != null) {
            ret.add(setPiecesTaker);
        }
        return ret;
    }
    ...
    public Vector<MatchLineupPosition> getReplacedPositions() {
        return replacedPositions;
    }

    public Vector<MatchLineupPosition> getRedCardedPositions() {
        return redCardedPositions;
    }
    ...
    /**
     * Player is no longer on the bench and must be removed from substitution
     * list if it was planned that he should replace another player
     */
    ...
    /**
     * Sets the provided list of match orders as list. list may contain a
     * maximum of three substitutions additional number of position swap and
     * behaviour changes depends on tactic assistant level list may contain one
     * man marking order
     *
    ...
    /**
     * Set a new man marking match order. An existing man marking order will be
    ...
    /**
     * Remove an existing man marking order. Does nothing if no man marking
     * order exists.
     */
        ...
        switch (s.getOrderType()) {
            ...
            case SUBSTITUTION -> {
                ...
                if (substitutedPlayer.isPresent()) {
                    ...
                    if (behaviour == -1) {
                        behaviour = substituted.getBehaviour();
                    }
    ...
    /**
     * ► IFK: off IFK = 0.5*mAtt + 0.3*mSP + 0.09*SPshooter def IFK = 0.4*mDef +
     * .3*mSP + 0.1*SPgk + 0.08*GKgk
     *
     * mAtt: outfield players attack skill average mDEf: outfield players
     * defence skill average
     */
