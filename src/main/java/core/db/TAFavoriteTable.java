...
final class TAFavoriteTable extends AbstractTable {
    ...
    protected void initColumns() {
        ...
        columns = new ColumnDescriptor[] {
                ColumnDescriptor.Builder.newInstance().setColumnName("TEAMID").setGetter((p) -> ((Team) p).getTeamId())
                        .setSetter((p, v) -> ((Team) p).setTeamId((int) v)).setType(Types.INTEGER).isNullable(false)
                        .isPrimaryKey(true).build(),
                ColumnDescriptor.Builder.newInstance().setColumnName("NAME").setGetter((p) -> ((Team) p).getName())
                        .setSetter((p, v) -> ((Team) p).setName((String) v)).setType(Types.VARCHAR).isNullable(true)
                        .setLength(20).build()
        };
    }
