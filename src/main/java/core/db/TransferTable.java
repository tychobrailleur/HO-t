...
public class TransferTable extends AbstractTable {
    ...
    protected void initColumns() {
        columns = new ColumnDescriptor[] {
                ColumnDescriptor.Builder.newInstance().setColumnName("transferid")
                        .setGetter((p) -> ((PlayerTransfer) p).getTransferId())
                        .setSetter((p, v) -> ((PlayerTransfer) p).setTransferId((int) v)).setType(Types.INTEGER)
                        .isPrimaryKey(true).isNullable(false).build(),
                ColumnDescriptor.Builder.newInstance().setColumnName("date")
                        .setGetter((p) -> ((PlayerTransfer) p).getDate().toDbTimestamp())
                        .setSetter((p, v) -> ((PlayerTransfer) p).setDate((HODateTime) v)).setType(Types.TIMESTAMP)
                        .isNullable(true).build(),
                ColumnDescriptor.Builder.newInstance().setColumnName("week")
                        .setGetter((p) -> ((PlayerTransfer) p).getWeek())
                        .setSetter((p, v) -> ((PlayerTransfer) p).setWeek((int) v)).setType(Types.INTEGER)
                        .isNullable(true).build(),
                ColumnDescriptor.Builder.newInstance().setColumnName("season")
                        .setGetter((p) -> ((PlayerTransfer) p).getSeason())
                        .setSetter((p, v) -> ((PlayerTransfer) p).setSeason((int) v)).setType(Types.INTEGER)
                        .isNullable(true).build(),
                ColumnDescriptor.Builder.newInstance().setColumnName("playerid")
                        .setGetter((p) -> ((PlayerTransfer) p).getPlayerId())
                        .setSetter((p, v) -> ((PlayerTransfer) p).setPlayerId((int) v)).setType(Types.INTEGER)
                        .isNullable(false).build(),
                ColumnDescriptor.Builder.newInstance().setColumnName("playername")
                        .setGetter((p) -> ((PlayerTransfer) p).getPlayerName())
                        .setSetter((p, v) -> ((PlayerTransfer) p).setPlayerName((String) v)).setType(Types.VARCHAR)
                        .setLength(127).isNullable(true).build(),
                ColumnDescriptor.Builder.newInstance().setColumnName("buyerid")
                        .setGetter((p) -> ((PlayerTransfer) p).getBuyerid())
                        .setSetter((p, v) -> ((PlayerTransfer) p).setBuyerid((Integer) v)).setType(Types.INTEGER)
                        .isNullable(true).build(),
                ColumnDescriptor.Builder.newInstance().setColumnName("buyername")
                        .setGetter((p) -> ((PlayerTransfer) p).getBuyerName())
                        .setSetter((p, v) -> ((PlayerTransfer) p).setBuyerName((String) v)).setType(Types.VARCHAR)
                        .setLength(256).isNullable(true).build(),
                ColumnDescriptor.Builder.newInstance().setColumnName("sellerid")
                        .setGetter((p) -> ((PlayerTransfer) p).getSellerid())
                        .setSetter((p, v) -> ((PlayerTransfer) p).setSellerid((Integer) v)).setType(Types.INTEGER)
                        .isNullable(true).build(),
                ColumnDescriptor.Builder.newInstance().setColumnName("sellername")
                        .setGetter((p) -> ((PlayerTransfer) p).getSellerName())
                        .setSetter((p, v) -> ((PlayerTransfer) p).setSellerName((String) v)).setType(Types.VARCHAR)
                        .setLength(256).isNullable(true).build(),
                ColumnDescriptor.Builder.newInstance().setColumnName("price")
                        .setGetter((p) -> ((PlayerTransfer) p).getPrice().getSwedishKrona())
                        .setSetter((p, v) -> ((PlayerTransfer) p).setPrice((AmountOfMoney) v)).setType(Types.DECIMAL)
                        .isNullable(true).build(),
                ColumnDescriptor.Builder.newInstance().setColumnName("tsi")
                        .setGetter((p) -> ((PlayerTransfer) p).getTsi())
                        .setSetter((p, v) -> ((PlayerTransfer) p).setTsi((Integer) v)).setType(Types.INTEGER)
                        .isNullable(true).build(),
                ColumnDescriptor.Builder.newInstance().setColumnName("motherclubfee")
                        .setGetter((p) -> ((PlayerTransfer) p).getMotherClubFee().getSwedishKrona())
                        .setSetter((p, v) -> ((PlayerTransfer) p).setMotherClubFee((AmountOfMoney) v))
                        .setType(Types.DECIMAL).isNullable(true).build(),
                ColumnDescriptor.Builder.newInstance().setColumnName("previousclubcommission")
                        .setGetter((p) -> ((PlayerTransfer) p).getPreviousClubFee().getSwedishKrona())
                        .setSetter((p, v) -> ((PlayerTransfer) p).setPreviousClubFee((AmountOfMoney) v))
                        .setType(Types.DECIMAL).isNullable(true).build()
        };
    }
    ...
    private final String getTransfersSql = createSelectStatement(
            " WHERE playerid = ? AND (buyerid=? OR sellerid=?) ORDER BY date DESC");

    /**
     * Gets a list of transfers.
     *
     * @param playerid     Player id for selecting transfers.
     * @param allTransfers If <code>false</code> this method will only return
     *                     transfers for your
     *                     own team, otherwise it will return all transfers for the
     *                     player.
     *
    ...
    public List<PlayerTransfer> getTransfers(int playerid, boolean allTransfers) {
        ...
        try {
            return load(PlayerTransfer.class,
                    this.connectionManager.executePreparedQuery(getAllTransfersSql, playerid));
        } catch (SQLException e) {
            HOLogger.instance().error(getClass(), e);
            return new ArrayList<>();
        }
    ...
    public List<PlayerTransfer> getTransfers(int teamId, int season, boolean bought, boolean sold) {
        ...
        try {
            return load(PlayerTransfer.class,
                    this.connectionManager.executePreparedQuery(createSelectStatement(sql), params.toArray()));
        } catch (SQLException e) {
            HOLogger.instance().error(getClass(), e);
            return new ArrayList<>();
        }
    ...
    public List<PlayerTransfer> getTeamTransfers(int teamId, boolean isSold) {
        ...
        try {
            return load(PlayerTransfer.class,
                    this.connectionManager.executePreparedQuery(createSelectStatement(sql), teamId));
        } catch (SQLException e) {
            HOLogger.instance().error(getClass(), e);
            return new ArrayList<>();
        }
    ...
    private final String getSumTransferCommissionsSql = "SELECT SUM(motherclubfee+previousclubcommission) FROM "
            + getTableName() + " WHERE date>=? AND date<?";
    ...
    public List<PlayerTransfer> getTransfersSince(Timestamp dbTimestamp) {
        try {
            return load(PlayerTransfer.class,
                    this.connectionManager.executePreparedQuery(getTransfersSinceSql, dbTimestamp));
        } catch (SQLException e) {
            HOLogger.instance().error(getClass(), e);
            return new ArrayList<>();
        }
