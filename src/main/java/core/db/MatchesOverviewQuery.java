...
class MatchesOverviewQuery {
	...
	static int getMatchesKurzInfoStatisticsCount(int teamId, int matchtype, int statistic) {
		...
		try (ResultSet rs = Objects.requireNonNull(DBManager.instance().getConnectionManager())
				.executePreparedQuery(sql.toString(), teamId, teamId)) {
			if (rs.next()) {
				tmp = rs.getInt("C");
			}
	...
	private static void fillMatchesOverviewChanceRow(boolean ownTeam, int teamId, MatchesHighlightsStat row,
			int iMatchType, MatchLocation matchLocation) {
		StringBuilder sql = new StringBuilder(200);
		ResultSet rs;
		List<Object> params = new ArrayList<>();
		sql.append(
				"SELECT MATCH_EVENT_ID, COUNT(*) AS C FROM MATCHHIGHLIGHTS JOIN MATCHESKURZINFO ON MATCHHIGHLIGHTS.MATCHID = MATCHESKURZINFO.MATCHID WHERE TEAMID");
		if (!ownTeam) {
			sql.append("!");
		}
		...
		try {
			rs = Objects.requireNonNull(DBManager.instance().getConnectionManager())
					.executePreparedQuery(sql.toString(), params.toArray());
		} catch (SQLException e) {
			HOLogger.instance().log(MatchesOverviewQuery.class, e);
			rs = null;
		}
	...
	private static StringBuilder createSubTypePlaceholders(List<MatchEvent.MatchEventID> subtypes,
			List<Object> params) {
		var sep = "";
	...
	private static StringBuilder getMatchLocationWhereClause(MatchLocation matchLocation, int teamId) {
		...
		switch (matchLocation) {
			case HOME ->
				sql.append(" AND HeimID=").append(teamId).append(" AND (isNeutral is NULL OR isNeutral=false) ");
			case AWAY ->
				sql.append(" AND GastID=").append(teamId).append(" AND (isNeutral is NULL OR isNeutral=false) ");
			case NEUTRAL -> sql.append(" AND (HeimID=").append(teamId).append(" OR GastID=").append(teamId)
	...
	static MatchesOverviewRow[] getMatchesOverviewValues(int matchtype, MatchLocation matchLocation) {
		...
		rows.add(new MatchesOverviewRow(Matchdetails.getNameForTaktik(IMatchDetails.TAKTIK_NORMAL),
				MatchesOverviewRow.TYPE_TACTICS, IMatchDetails.TAKTIK_NORMAL));
		rows.add(new MatchesOverviewRow(Matchdetails.getNameForTaktik(IMatchDetails.TAKTIK_PRESSING),
				MatchesOverviewRow.TYPE_TACTICS, IMatchDetails.TAKTIK_PRESSING));
		rows.add(new MatchesOverviewRow(Matchdetails.getNameForTaktik(IMatchDetails.TAKTIK_KONTER),
				MatchesOverviewRow.TYPE_TACTICS, IMatchDetails.TAKTIK_KONTER));
		rows.add(new MatchesOverviewRow(Matchdetails.getNameForTaktik(IMatchDetails.TAKTIK_MIDDLE),
				MatchesOverviewRow.TYPE_TACTICS, IMatchDetails.TAKTIK_MIDDLE));
		rows.add(new MatchesOverviewRow(Matchdetails.getNameForTaktik(IMatchDetails.TAKTIK_WINGS),
				MatchesOverviewRow.TYPE_TACTICS, IMatchDetails.TAKTIK_WINGS));
		rows.add(new MatchesOverviewRow(Matchdetails.getNameForTaktik(IMatchDetails.TAKTIK_CREATIVE),
				MatchesOverviewRow.TYPE_TACTICS, IMatchDetails.TAKTIK_CREATIVE));
		rows.add(new MatchesOverviewRow(Matchdetails.getNameForTaktik(IMatchDetails.TAKTIK_LONGSHOTS),
				MatchesOverviewRow.TYPE_TACTICS, IMatchDetails.TAKTIK_LONGSHOTS));
		rows.add(new MatchesOverviewRow(TranslationFacility.tr("ls.team.teamattitude"), MatchesOverviewRow.TYPE_TITLE));
		rows.add(new MatchesOverviewRow(Matchdetails.getNameForEinstellung(IMatchDetails.EINSTELLUNG_PIC),
				MatchesOverviewRow.TYPE_MOT, IMatchDetails.EINSTELLUNG_PIC));
		rows.add(new MatchesOverviewRow(Matchdetails.getNameForEinstellung(IMatchDetails.EINSTELLUNG_NORMAL),
				MatchesOverviewRow.TYPE_MOT, IMatchDetails.EINSTELLUNG_NORMAL));
		rows.add(new MatchesOverviewRow(Matchdetails.getNameForEinstellung(IMatchDetails.EINSTELLUNG_MOTS),
		...
		rows.add(new MatchesOverviewRow("IMatchDetails.WETTER_SONNE", MatchesOverviewRow.TYPE_WEATHER,
				Weather.SUNNY.getId()));
		rows.add(new MatchesOverviewRow("IMatchDetails.WETTER_WOLKIG", MatchesOverviewRow.TYPE_WEATHER,
				Weather.PARTIALLY_CLOUDY.getId()));
		rows.add(new MatchesOverviewRow("IMatchDetails.WETTER_BEWOELKT", MatchesOverviewRow.TYPE_WEATHER,
				Weather.OVERCAST.getId()));
		rows.add(new MatchesOverviewRow("IMatchDetails.WETTER_REGEN", MatchesOverviewRow.TYPE_WEATHER,
				Weather.RAINY.getId()));
	...
	private static StringBuilder getMatchLocationWhereClause(MatchLocation matchLocation, int teamId, boolean home) {
		...
		switch (matchLocation) {
			case HOME:
				sql.append(" AND (isNeutral is NULL OR isNeutral=false) AND HeimID=").append(teamId);
				break;
			case AWAY:
				sql.append(" AND (isNeutral is NULL OR isNeutral=false) AND GastID=").append(teamId);
				break;
			case NEUTRAL:
				sql.append(" AND isNeutral=true");
			case ALL:
				sql.append(" AND ").append(home ? "HEIMID=" : "GASTID=").append(teamId);
				break;
		}
		return sql;
	}
