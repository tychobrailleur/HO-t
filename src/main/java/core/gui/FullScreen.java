...
class FullScreen {
	 ...
	 * 
	 * @param frame the frame that is manipulated and displayed in normal windowed
	 *              or full screen mode
	 */
	void init(JFrame frame) {
		if (!isInitialized) {
			...
			for (GraphicsDevice graphicsDevice : devices) {
				...
				// HOLogger.instance().debug(getClass(), "FullScreen: "+confs.length+" configs
				// for dev "+devices[i].getIDstring()+" found,
				// iFSS="+devices[i].isFullScreenSupported());
				for (GraphicsConfiguration conf : confs) {
					// HOLogger.instance().debug(getClass(), "FullScreen: Conf found:
					// "+k+"="+confs[k].toString());
					if (frameConf == conf) {
						// HOLogger.instance().debug(getClass(), "FullScreen: Matching conf found:
						// ["+k+"]="+confs[k].toString()+" for dev["+i+"]="+devices[i].getIDstring());
			...
			try {
				...
				// HOLogger.instance().debug(getClass(), "FullScreen: dev
				// "+device.getIDstring()+",
				// isFullScreenSupported="+device.isFullScreenSupported());
			} catch (Exception e) {
	...
	/**
	 * Restore normal mode and stops the non daemon thread. Should be called on
	 * exit.
	 * 
	 * @param frame the frame that is manipulated and displayed in normal windowed
	 *              mode
	...
	/**
	 * Returning to normal windowed mode. It is save to call it when not in full
	 * screen mode.
	 * 
	 * @param frame the frame that is manipulated and displayed in normal windowed
	 *              mode
	...
	void startBlockingThread() {
		...
		Runnable r = () -> {
			...
			try {
				...
				// HOLogger.instance().debug(getClass(), "FullScreen: Blocking thread terminated
				// (interrupted).");
			}
			// HOLogger.instance().debug(getClass(), "FullScreen: Blocking thread
			// finished.");
		};
