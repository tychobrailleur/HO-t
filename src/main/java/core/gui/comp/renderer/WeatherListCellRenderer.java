...
public class WeatherListCellRenderer implements ListCellRenderer<Object> {
    ...
    public ColorLabelEntry m_clEntry = new ColorLabelEntry("", ColorLabelEntry.FG_STANDARD,
            ThemeManager.getColor(HOColorName.BACKGROUND_CONTAINER), SwingConstants.LEFT);

    public javax.swing.JLabel m_jlLeer = new javax.swing.JLabel(" ");

    @Override
    public final java.awt.Component getListCellRendererComponent(JList jList, Object obj, int index, boolean isSelected,
            boolean cellHasFocus) {
        if (obj instanceof CBItem item) {
            int id = item.getId();
            Icon icon;
            if (id == 4) {
