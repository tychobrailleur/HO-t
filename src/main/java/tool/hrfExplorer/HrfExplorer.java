...
public class HrfExplorer extends ImagePanel
		implements ActionListener, ItemListener, MouseListener, IRefreshable, TableColumnModelListener {
	...
	@SuppressWarnings("unchecked")
	private static Hashtable m_HashTable_DayInDB = new Hashtable(40); // KEY: Tag des gewählten Monats in Calendar,
																		// VALUE: HRF-ID für diesen Tag
	@SuppressWarnings("unchecked")
	private Hashtable m_HashTable_Details = new Hashtable(40); // KEY: Pfad oder Datum eines HrfDetails-Objekt, VALUE:
																// das HrfDetails-Objekt
	@SuppressWarnings("unchecked")
	private Hashtable m_HashTable_Details_ColHeader = new Hashtable(40); // KEY: Datum eines HrfDetails-Objekt VALUE:
																			// das HrfDetails-Objekt
	@SuppressWarnings("unchecked")
	private Hashtable m_HashTable_Columns = new Hashtable(40); // KEY: Spaltenname der Detailtabelle VALUE: Vector mit
																// dem Inhalt einer Spalte der Detailtabelle
	@SuppressWarnings("unchecked")
	private static Hashtable m_HashTable_DatumKey = new Hashtable(40); // KEY: Datum im Format YYYY-MM-DD VALUE:
																		// Dateipfad
	@SuppressWarnings("unchecked")
	private Hashtable m_HashTable_Import = new Hashtable(40); // KEY: Pfad der Dateien aus der Importtabelle VALUE: ---
	@SuppressWarnings("unchecked")
	private static Hashtable m_HashTable_isEvent = new Hashtable(40); // KEY: Tag des gewählten Monats in Calendar,
																		// VALUE: Matchtyp als String
	@SuppressWarnings("unchecked")
	...
	private void initialize() {
		...
		try {
			...
			// tmp.add(new JLabel("An error occured while loading language.properties.
			// Please report to me."));
			// tmp.show();
		}
		 ...
		 */
		var importHeader = new Vector();
		var importValues = new Vector();
		...
		m_int_Hoehe_DetailPanels = m_Ar_Detail_Label_fix.length * m_int_Hoehe_Label; // Festlege der Gesamthöhe des
																						// Detailpanels
		...
		var fixedPanel = new HrfPanel(m_int_Breite_Detail_Fixed, m_int_Hoehe_DetailPanels);
		...
		for (int ii = 0; ii < m_Ar_Detail_Label_fix.length; ii++) {
			...
			fixedPanel.add(createLabel((m_Ar_Detail_Label_fix[ii] + " :"), m_int_Breite_Detail_Fixed, m_int_Hoehe_Label,
					JLabel.RIGHT, m_LineColor));
		}

		var emptyPanel = new HrfPanel(m_int_Breite_Detail_Fixed, m_int_Hoehe_Label + 4);
		var emptyLabel = createLabel("", m_int_Breite_Detail_Fixed, m_int_Hoehe_Label, SwingConstants.CENTER, null);
		emptyPanel.add(emptyLabel);

		var fixedColumn = new Vector();
		...
		var colFixed = m_Table_Details.getColumnModel().getColumn(0);
		colFixed.setPreferredWidth(m_int_Breite_Detail_Fixed);

		var kante = BorderFactory.createBevelBorder(BevelBorder.RAISED, hellblau, dunkelblau);
	...
	public void createCalendarTable(int monat, int jahr) {
		...
		// Holen der HRF-ID und des Datums der Einträge, die in dem gewählten Monat
		// liegen
		doSelect("SELECT DATUM,HRF_ID FROM HRF where DATUM between '" + jahr_Start + "-" + monat_Start + "-01' and '"
				+ jahr_Ende + "-" + monat_Ende + "-01'");
		// ********************************************************************************************
		...
		var gc = new GregorianCalendar(m_int_selectedYear, m_int_selectedMonth, 1);
		 ...
		 */
		var cStart = new GregorianCalendar(Integer.parseInt(jahr_Start), Integer.parseInt(monat_Start) - 1, 1);
		var tsStart = new Timestamp(cStart.getTimeInMillis());
		var cStop = new GregorianCalendar(Integer.parseInt(jahr_Ende), Integer.parseInt(monat_Ende) - 1, 1);
		var tsStop = new Timestamp(cStop.getTimeInMillis());
		doSelect("SELECT SPIELDATUM,MATCHID FROM MATCHDETAILS WHERE SPIELDATUM between '" + tsStart + "' AND '" + tsStop
				+ "' AND ( GASTID = '" + m_TeamID + "' OR HEIMID = '" + m_TeamID + "' )");
		...
		while (actual_day <= last_day) {
			var tmp = new Vector();
			var gc_tmp = new GregorianCalendar(m_int_selectedYear, m_int_selectedMonth, actual_day);
	 ...
	 * 
	 * @param hashwert Dieser Parameter ist der Key, mit dem das HrfDetails-Object
	 *                 aus der Hashtable m_HashTable_Details geholt wird
	 ******************/
	@SuppressWarnings("unchecked")
	public void createDetailTable(String hashwert) {
		...
		if (!m_HashTable_Details_ColHeader.containsKey(selectedObject.getDatum())) {
			...
			String[] objectDetails = {
					...
					" " + selectedObject.getPunkte() + " / " + selectedObject.getToreFuer() + ":"
							+ selectedObject.getToreGegen(),
			...
			for (int ii = 0; ii < objectDetails.length; ii++) {
				...
				teamDetails.add(createLabel(objectDetails[ii], m_int_Breite_Detail_Var, m_int_Hoehe_Label, JLabel.LEFT,
						m_LineColor));
			}
			...
			JLabel remove = createLabel(TranslationFacility.tr("ls.button.remove"), m_int_Breite_Detail_Var,
					m_int_Hoehe_Label, JLabel.CENTER, rot);
			remove.getInsets();
	...
	/******************
	 * Liefert ein boolean Ergebnis, ob für einen Tag ein HRF-File in der DB ist und
	 * wertet dabei die Hashtable aus
	 * 
	 * @param tag Der Tag, der geprüft werden soll
	 * @return Ein boolean-Wert, der anzeigt, ob für den übergebenen Tag ein
	 *         DB-Eintrag existiert
	 ******************/
	...
	/******************
	 * Liefert ein boolean Ergebnis, ob an einem Tag ein Spiel oder Training
	 * stattgefunden hat
	 * 
	 * @param tag Der Tag, der geprüft werden soll
	 * @return Ein boolean-Wert, der anzeigt, ob an dem übergebenen Tag ein
	 *         SpecialEvent (z.B. ein Spiel) stattgefunden hat.
	 ******************/
	 ...
	 * @return Ein boolean-Wert, der anzeigt, ob eine gewählte Datei bereits in der
	 *         Datenbank existiert.
	 ******************/
	...
	public void actionPerformed(ActionEvent e) {
		 ...
		 */
			...
			if (anzRows > 0) {
				for (int ii = 0; ii < anzRows; ii++) {
					...
					// String dateiPfad =
					// m_TableModel_Filelist.getValueAt(ii,m_TableModel_Filelist.getColumnCount()).toString();
		 ...
		 */
			...
			for (int ii = 0; ii < anzRows; ii++) {
				if ((Boolean) m_TableModel_Filelist.getValueAt(ii, 0)) {
					...
					// String delete_key = "" +
					// m_TableModel_Filelist.getValueAt(ii,m_TableModel_Filelist.getDataVector().capacity());
		 ...
		 */
			...
			// Ein oder mehrere ausgewählte Files werden physikalisch von der Festplatte
			// entfernt
			int anzRows = m_TableModel_Filelist.getRowCount();
			...
			for (int i = 0; i < anzRows; i++) {
				if ((Boolean) m_TableModel_Filelist.getValueAt(i, 0)
					...
					// String deletePath = "" +
					// m_TableModel_Filelist.getValueAt(i,m_TableModel_Filelist.getDataVector().capacity());
					...
					int option = JOptionPane.showConfirmDialog(null,
							TranslationFacility.tr("deletefile") + "\n" + deletePath,
							TranslationFacility.tr("deletefile"), JOptionPane.YES_NO_OPTION);
					if (option == 0) {
		 ...
		 */
			...
			for (int i = 0; i < anzRows; i++) {
				if ((Boolean) m_TableModel_Filelist.getValueAt(i, 0)
					...
					// int deleteHRF_ID =
					// ((Integer)(m_TableModel_Filelist.getValueAt(i,m_TableModel_Filelist.getDataVector().capacity()))).intValue();
	...
	public void refresh() {
		...
		// debugWindow.append("Anzahl Keys in m_V_Filelist_Keys: " +
		// m_V_Filelist_Keys.size());
